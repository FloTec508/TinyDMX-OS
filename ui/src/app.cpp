...
void App::init()
{
    ...
    QWidget *container = new QWidget(this);   // Create container widget
    QVBoxLayout *mainLayout = new QVBoxLayout(container);  // Create vertical layout

    m_tab = new QTabWidget(container);
    m_tab->setTabPosition(QTabWidget::South);

    m_bottombox = new QVBoxLayout(this);


    mainLayout->addWidget(m_tab); // Add the tab widget at the top
    QHBoxLayout* faderLayout = new QHBoxLayout();
    QHBoxLayout* encoderLayout = new QHBoxLayout();

    for(int i = 0; i < 6; i++){
        QFrame* frame = new QFrame();
        frame->setStyleSheet("QFrame { background: #333; border: 1px solid #666; border-radius:4px;}");
        frame->setMinimumSize(70,70);
        QVBoxLayout* frameLayout = new QVBoxLayout(frame);
        m_faderLayouts.append(frameLayout);
        QLabel* label = new QLabel();
        label->setText("Master " + QString::number(i + 1));
        label->setStyleSheet("color: white; border:none; font-wheight: bold; padding:0px;");
        label->setAlignment(Qt::AlignCenter);
        frameLayout->addWidget(label);
        faderLayout->addWidget(frame);
    }

    for(int i = 0; i < 4; i++){
        QFrame* frame = new QFrame();
        frame->setStyleSheet("QFrame { background: #333; border: 1px solid #666; }");
        //frame->setMinimumSize(20,70);
        frame->setMaximumHeight(50);
        frame->setMinimumHeight(40);
        QVBoxLayout* frameLayout = new QVBoxLayout(frame);
        QLabel* titleLabel = new QLabel();
        QLabel* subLabel = new QLabel();
        titleLabel->setStyleSheet("color: white; border:none; font-wheight: extrabold; padding:0px;");
        titleLabel->setText("Encoder " + QString::number(i + 1));
        titleLabel->setAlignment(Qt::AlignCenter);
        subLabel->setStyleSheet("color: white; border:none; padding:0px;");
        subLabel->setText("value");
        subLabel->setAlignment(Qt::AlignCenter);
        m_encoderTitles.append(titleLabel);
        m_encoderSubtitles.append(subLabel);

        frameLayout->addWidget(titleLabel);
        frameLayout->addWidget(subLabel);

        encoderLayout->addWidget(frame);
    }
    ...
    m_bottombox->addWidget(m_commandline);
    m_bottombox->addLayout(encoderLayout);
    m_bottombox->addLayout(faderLayout);

    connect(m_commandline, &QLineEdit::returnPressed, this, &App::slotCommandEnter);

    mainLayout->addLayout(m_bottombox);
    ...
    QWidget* w = new VirtualConsole(m_tab, m_doc);
    m_tab->addTab(w, QIcon(":/virtualconsole.png"), tr("Home"));
    ...
    //this->setStyleSheet("QWidget {background-color: #121212;color: #ffffff;border: none;}QPushButton {background-color: #1e1e1e;border: 2px solid #333;border-radius: 4px;padding: 6px;}QPushButton:hover {background-color: #292929;}QPushButton:pressed {background-color: #3a3a3a;}QLineEdit {background-color: #1e1e1e;border: 1px solid #444;padding: 4px;}");

    QFile file(":/style.qss");  // Load from Qt resource system
    if (file.open(QFile::ReadOnly | QFile::Text))
    {
        QTextStream stream(&file);
        this->setStyleSheet(stream.readAll());
        qInfo() << "Loaded Stylesheet";
    }
    else
    {
...
void App::slotCommandEnter()
{
    QString input = m_commandline->text().trimmed();
    m_commandline->clear();

    CommandLineEdit cmd = CommandLineEdit();
